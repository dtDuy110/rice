...
export default function CartPage() {
  ...
  const shipping = items.length > 0 ? 30000 : 0  // 30,000 ₫
  const tax = subtotalAfterDiscount * 0.08  // 8% VAT
  const total = subtotalAfterDiscount + shipping + tax

  const handleApplyDiscount = () => {
    if (discountCode.trim().toUpperCase() === 'GIAMGIA10') {
  ...
  return (
    <div className="py-8 md:py-12 px-4 md:px-12 max-w-[1280px] mx-auto animate-fade-in">
      <Breadcrumbs />
      <h1 className="text-headline-lg text-on-surface mb-8" style={{ fontFamily: 'var(--font-family-heading)', fontSize: '36px', fontWeight: 700 }}>
        Giỏ hàng
      </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {items.map(item => {
            ...
            return (
              <div key={product._id} className="bg-surface rounded-2xl p-6 border border-surface-variant shadow-[var(--shadow-card)] flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
                ...
                <span className="text-primary font-bold text-xl shrink-0" style={{ fontFamily: 'var(--font-family-heading)' }}>{formatVND((product.price || 0) * item.quantity)}</span>
              </div>
            )
          })}
        ...
        <div className="bg-surface rounded-2xl p-8 border border-surface-variant shadow-[var(--shadow-card)] h-fit sticky top-24">
          ...
          <div className="space-y-4 mb-6">
            ...
            {discountAmount > 0 && (
              <div className="flex justify-between text-body-md"><span className="text-primary font-medium">Giảm giá ({discountPercent}%)</span><span className="text-primary font-medium">-{formatVND(discountAmount)}</span></div>
            )}
            ...
            <div className="flex justify-between text-body-md"><span className="text-on-surface-variant">Thuế (VAT 8%)</span><span>{formatVND(tax)}</span></div>
